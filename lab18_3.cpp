...
student* findstudent(vector<student>& allstudents, int key) { //แก้ไข function signature
...
int main() {
    ...
    while(getline(student_file, textline)) {
        ...
        char name[100];
        char gender;
        sscanf(textline.c_str(), "%[^,],%d,%c,%f", name, &s.id, &gender, &s.gpa);
        s.name = name;
        s.gender = string(1, gender); // แปลง char เป็น string
